...
- `cli.py` — wires the three stages together (commands replaced, *then* prefixes stripped — order matters since collision-skipped jobs keep their prefixed names and thus keep any comment appended in the command step), configures `logging.basicConfig` to append to `LOG_FILE` (`f"{PROG_NAME}.log"`) at `--log-level`, and reports three counts: jobs read, jobs updated (command replaced), jobs renamed (prefix stripped). These are independent counters — a job can be counted in either, both, or neither.
